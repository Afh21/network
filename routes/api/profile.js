...
router.get("/user/:user_id", (req, res) => {
  profile
    ...
    .catch(err =>
      res.status(404).json({ profile: "There isn´t profile for this user" })
    );
});
...
router.get("/all", (req, res) => {
  ...
  profile
    ...
    .populate("user")
    .then(profiles => {
      errors.noprofile = "There aren´t profiles";
      if (!profiles) return res.status(404).json(errors);

      res.json(profiles);
    })
    .catch(() => res.json({ profile: "There aren´t profiles " }));
});
  ...
  (req, res) => {
    ...
    profile
      ...
      .then(prof => {
        ...
        prof.save().then(profile => res.json(profile));
      });
  }
);
...
router.delete(
  ...
  (req, res) => {
    profile
      ...
      .then(prof => {
        // Get remove index
        const removeIndex = prof.experience
          .map(item => item.id)
